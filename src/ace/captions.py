...
def plan(generation: str | Path, workspace: str | Path | None = None, *, mood: str | None = None) -> list[CaptionCue]:
    ...
    evidence_plan = read_json(folder / "evidence" / "evidence-plan.json", []) or []
    has_evidence = bool(evidence_plan.get("items") if isinstance(evidence_plan, dict) else evidence_plan)
